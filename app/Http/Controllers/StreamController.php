...
class StreamController extends Controller
{
  ...
  public function getTwitchStreamDetails(Request $request)
{
    try {
        ...
        $livestream = Livestream::where('stream_id', $response['data'][0]['id'])
            ->where('type', 'twitch')
            ->first();
        ...
        $game_name = $response['data'][0]["game_name"];
        $game_name = $game_name == "Apex Legends" ? "Apex Legends" :
                    ($game_name == "Counter-Strike: Global Offensive" ? "CSGO" : $game_name);
        ...
        $game_id = $response['data'][0]['user_id'] ?? 0;
        ...
        $count_label = $chk_label = $label_name = $l_name = $user_room_names = $current_room_names = $rm_name = '';
        $count_bet = 0;
        ...
        if ($user_id) {
            ...
            if (Bet::where('user_id', $user_id)->count() > 0) {
                $chk_first_bet = Bet::select('user_id', 'id', 'game_id')->where('game_id', $livestream->id)
                ...
                if ($bet_user_id == $user_id) {
                    ...
                    $r = GameRoom::with('game_room')->where('game_id', $livestream->id)->first();
                    if ($r) {
                        $rm_name = $r->game_room->room_name;
                        $count_bet = 3;
                    } else {
        ...
        Log::error('Error retrieving Twitch stream details: ' . $e->getMessage());

        return response()->json([
            'status' => 'error',
            'message' => 'Failed to retrieve Twitch stream details.',
        ], 500);
    }
}
