...
class LiveStreamIndex extends Controller
{
    ...
    {
        try {
            // Retrieve in-progress livestreams
            $livestreams = Livestream::whereNull('type')
                ->whereIn('status', ['started', 'created'])
                ->with('user')
            ...
            // Log the error
            Log::error('Error fetching in-progress livestreams: ' . $e->getMessage());

            // Return an error response
            return response()->json([
                'status' => 'error',
                'error' => 'An error occurred while fetching in-progress livestreams.'
            ], 500);
        }
    }

    public function GetCompletedStreams(Request $request)
    {
        try {
            // Retrieve in-progress livestreams
            $livestreams = Livestream::whereNull('type')
                ->whereIn('status', ['stopped'])
                ->with('user')
                ->orderBy('created_at', 'desc')
                ->get();
    ...
    public function GetStreamingReport(Request $request)
{
    try {
        ...
        $users = Users::whereHas('streams', function ($query) use ($fromDate, $toDate) {
            $query->whereBetween('created_at', [$fromDate, $toDate]);
        })->with(['streams' => function ($query) use ($fromDate, $toDate) {
            $query->whereBetween('created_at', [$fromDate, $toDate])->withCount('bets')->withSum('bets', 'amount')->withSum('bets', 'vig_amount');
        }, 'streams.bets'])->get();

        $data = [];

        foreach ($users as $user) {
            $streamer_fee = round(($user->streams->sum('bets_sum_vig_amount') * $setting->streamer_per) / 100);
            $profit = round(($user->streams->sum('bets_sum_vig_amount') - $streamer_fee));
            $data[] = [
                'streamer_name' => $user->username,
                'total_stream' => $user->streams->count(),
                'total_bet_count' => $user->streams->sum('bets_count'),
                'total_amount' => $user->streams->sum('bets_sum_amount'),
                'total_vig_amount' => $user->streams->sum('bets_sum_vig_amount'),
                'streamer_fee' => $streamer_fee,
                'profit' => $profit
            ];
        }
        ...
        return response()->json(['status' => 'error', 'message' => 'Failed to fetch report'], 500);
    }
}
