...
if ($user) {
    ...
    $userAge = $dateOfBirth ? now()->diffInYears($dateOfBirth) : null;
} else {
    $userAge = null;
}
@endphp
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        Swal.fire({
            title: 'Information',
            text: `Since you're under 21, you can't place bets. However, you can still watch live streams.`,
            icon: 'info',
      ...
      <section class="py-0 py-lg-5">
         <div class="container">
            ...
            <div class="section-authentication-signin1 d-flex align-items-center justify-content-center my-5 my-lg-0">
               <video id="player" class="video-js" controls autoplay muted preload="auto" data-setup="{}">
                         ...
                         <div class="col-md-10">
                          ...
                          <div class="col-md-2">
                            ...
                            <div class="mb-3">
                              ...
                              <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->fullUrl()) }}" target="_blank" class="btn btn-primary">
                                  <i class='bx bxl-facebook-square'></i>
                              </a>

                              <!-- WhatsApp Share Button -->
                              <a href="https://wa.me/?text={{ urlencode(request()->fullUrl()) }}" target="_blank" class="btn btn-success">
                                  <i class='bx bxl-whatsapp'></i>
                              </a>

                              <!-- Reddit Share Button -->
                              <a href="https://www.reddit.com/submit?url={{ urlencode(request()->fullUrl()) }}" target="_blank" class="btn btn-danger">
                                  <i class='bx bxl-reddit'></i>
                              </a>

                              <!-- Twitter Share Button -->
<a href="https://twitter.com/intent/tweet?text=Check%20out%20this%20link:&url={{ urlencode(request()->fullUrl()) }}" target="_blank" class="mt-2 btn btn-info">
    <i class='bx bxl-twitter'></i>
</a>
<!-- LinkedIn Share Button -->
<a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(request()->fullUrl()) }}" target="_blank" class="mt-2 btn btn-primary">
    <i class='bx bxl-linkedin'></i>
</a>
<!-- Pinterest Share Button -->
<a href="https://www.pinterest.com/pin/create/button/?url={{ urlencode(request()->fullUrl()) }}&media=&description=" data-pin-do="buttonPin" data-pin-custom="true" class="mt-2 btn btn-danger">
    <i class='bx bxl-pinterest'></i>
</a>
                          </div>
                          <div class="mb-3">
                           @auth
                           @include('live/stream_like')
                           @endauth
                           </div>
                           <div class="mb-3">
                           @if ($email > 0 && $email != '')
                            @include('live/stream_report') 
                            @endif
                            </div>
                                     ...
                                     @endphp -->
                                        ...
                                        @else
                                            ...
                                            <button type="button" class="btn btn-success" disabled>
                                               ...
                                               <p class="mt-1"><i class='bx bxs-info-circle'></i> Restrict for user below 21 year's </p>  
                                        @endif

                                    @else

                                        <a href="{{ route('login') }}" class="btn btn-danger">Create Own Bet</a>
                                        <a href="{{ route('login') }}" class="btn btn-success"> Purchase ELO</a>

                                        <p class="mt-1"><i class='bx bxs-info-circle'></i> To use Betting & Purchase functionality you have to register / login </p>
                                    @endauth
                     ...
                     <div class="row">
                         @auth
                                 ...
                                 @else
                                     List of Active Bets restrict for user below 21 year's
                                 @endif
                             @endauth
                     </div>  
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
         <div class="modal-content" style="background-color: #13143e;">
            ...
            <div class="modal-footer">
               ...
               <button type="button" value="button" class="btn btn-primary" id="submit_new_bet">Submit</button>
            </div>
         </div>
      </div>
   </form>
...
<div class="modal fade" id="activebetsmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
   <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
         <div class="modal-header">
            I know I only have room for me as your wedding is on loan
...
@section('js')
<!-- <script src="https://elo-esports.com/js/app.js"></script> -->
  ...
  $decryptedClientId = null;
   try {
       if($setting->client_id){
         $decryptedClientId = \Crypt::decryptString($setting->client_id);
       }  
    } catch (\Illuminate\Contracts\Encryption\DecryptException $e) {
      
    }
@endphp
<script src="https://www.paypal.com/sdk/js?client-id={{ $decryptedClientId }}&currency=USD"></script>
...
<script type="text/javascript">
 ...
 $(document).ready(function() {
   $('body').on('click', '#submit_report_form', function () {
    //$('#reportForm').on('submit', function(e) {
       ...
       var report_for = $('input[name="report_for"]:checked').val();
       var stream_id = '';
        var stream_type = '';

        // Extract stream_id and stream_type from URL
        var url = window.location.href;
        if (url.includes('/stream/')) {
           
            var parts = url.split('/stream/');
            if (parts.length === 2) {
                stream_id = parts[1];
                stream_type = 'home_stream';
            }
        } else if (url.includes('/streams/')) {
         
            var parts = url.split('/streams/');
            if (parts.length === 2) {
                stream_id = parts[1];
                stream_type = 'twitch_stream';
            }
        }
       ...
       report_for = report_for ? report_for.trim() : '';
       ...
       var errors = false;
       if (!name) {
           $('#reportNameError').text('Name is required.');
           errors = true;
       } else {
           $('#reportNameError').text('');
       }

       if (!mobile) {
           $('#reportMobileError').text('Mobile is required.');
           errors = true;
       } else if (!/^\d{10}$/.test(mobile)) {
           $('#reportMobileError').text('Invalid mobile number.');
           errors = true;
       } else {
           $('#reportMobileError').text('');
       }

       if (!email) {
           $('#reportEmailError').text('Email is required.');
           errors = true;
       } else if (!/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/.test(email)) {
           $('#reportEmailError').text('Invalid email address.');
           errors = true;
       } else {
           $('#reportEmailError').text('');
       }

        if (!report_for) {
           $('#reportforError').text('Please choose an option.');
           errors = true;
       } else {
           $('#reportforError').text('');
       }
       ...
       if (!remark) {
           $('#reportRemarkError').text('remark is required.');
           errors = true;
       } else {
           $('#reportRemarkError').text('');
       }
      ...
      var formData = $('#reportForm').serialize(); 
        ...
        formData += '&stream_type=' + encodeURIComponent(stream_type);

       $.ajax({
            url: "{{route('save-report')}}",
           type: 'POST',
           data: formData,
           success: function(response) {
               // Handle success
               console.log(response);
               alert("Report sumbited successfully");
